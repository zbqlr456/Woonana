<template>
  <div class="bottom-nav">
    <div v-if="window.location.href === `${VUE_APP_LOCAL_URI.AllBlogs}`" class="nav-item-fill">
      <router-link to="/AllBlogs">
        <img src="@/assets/images/icon/house-fill.svg" class="nav-item-img-fill" />
        <p class="nav-item-name-fill">홈</p>
      </router-link>
    </div>
    <div v-else class="nav-item">
      <router-link to="/AllBlogs">
        <img src="@/assets/images/icon/house.svg" class="nav-item-img" />
        <p class="nav-item-name">홈</p>
      </router-link>
    </div>
    <div class="nav-item">
      <router-link to="/newpost">
        <img src="@/assets/images/icon/plus-square-fill.svg" class="nav-item-img" />
        <p class="nav-item-name">글 작성</p>
      </router-link>
    </div>
    <div class="nav-item">
      <router-link to="/chat/chatroomlist">
        <img src="@/assets/images/icon/chat-dots-fill.svg" class="nav-item-img" />
        <p class="nav-item-name">채팅</p>
      </router-link>
    </div>
    <div class="nav-item">
      <router-link to="/mypage">
        <img src="@/assets/images/icon/person-fill.svg" class="nav-item-img" />
        <p class="nav-item-name">마이</p>
      </router-link>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style>
.bottom-nav {
  position: fixed;
  display: flex;
  bottom: 0;
  left: 0;
  border-top: 1px solid;
  border-color: rgb(195, 195, 195);
  background-color: #ffffff;
  width: 100%;
  height: 8%;
  justify-content: space-around;
}
.nav-item-fill {
  display: flex;
  justify-content: center;
  align-items: center;
  text-align: center;
}
.nav-item {
  display: flex;
  justify-content: center;
  align-items: center;
  text-align: center;
}
.nav-item-img-fill {
  width: 20px;
  height: 20px;
}
.nav-item-img {
  width: 20px;
  height: 20px;
}
.nav-item-name-fill {
  font-size: 11px;
  margin-bottom: -8px;
}
.nav-item-name {
  font-size: 11px;
  margin-bottom: -8px;
}
</style>
